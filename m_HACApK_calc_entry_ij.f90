...
real(kind=8) function HACApK_entry_ij(i,j,zbemv)
	...
	real(kind=8) function loop_Biot_Savart(i, j,zbemv)
	real(kind=8) :: X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3, X4, Y4, Z4
	real(kind=8), dimension(3) :: H12, H23, H34, H41, H
	integer(kind=4) :: i, j
	type(st_HACApK_calc_entry) :: zbemv
	real(kind=8),pointer,dimension(:) :: coil_x, coil_y, coil_z
	real(kind=8),pointer,dimension(:) :: eval_x, eval_y, eval_z
	...
	function Biot_Savart(Ox, Oy, Oz, X1, Y1, Z1, X2, Y2, Z2)
		real(kind=8) Biot_Savart(3)
		real(kind=8) :: X1, Y1, Z1, X2, Y2, Z2
		real(kind=8) :: Ox, Oy, Oz
		real(kind=8) :: X21, Y21, Z21, R21
		real(kind=8) :: OxX1, OyY1, OzZ1, OR1,OxX2, OyY2, OzZ2, OR2

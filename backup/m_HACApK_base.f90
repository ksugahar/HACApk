...
!	use mpi
	...
	st_ctl%param(8) =1;				! 10:BLR, 20:BH, other:Leafmtx
	...
	st_ctl%param(43)=5;				! BH : Number of blocks with a color in a row of process grid
	...
	st_ctl%param(91)=1.0e-5		! Required accuracy iterative solver
...
! stop
	...
	irank_t=st_ctl%lpmd(33); irank_l=st_ctl%lpmd(37)
	do in=0,nlfalt-1
		nml=mod(in,npgl); inmt=mod(in,npgt)
		if(inmt==irank_t) st_leafmtxp%lbl2t(inml)=1
	enddo
	if(st_ctl%param(1)>1) write(mpilog,*) 'st_leafmtxp%lbl2t'
	if(st_ctl%param(1)>1) write(mpilog,*) st_leafmtxp%lbl2t(:) 
